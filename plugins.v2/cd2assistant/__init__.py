...
class Cd2Assistant(_PluginBase):
    ...
    def init_plugin(self, config: dict = None):
        ...
        if not CLOUDDRIVE_AVAILABLE:
            ...
            self.systemmessage.put("Cd2助手启动失败：缺少 clouddrive 依赖库，请安装：pip install clouddrive")
            return
        ...
        if self._enabled or self._onlyonce or self._cd2_restart:
            ...
            for cd2_conf in self._cd2_confs.split("\n"):
                ...
                if not _cd2_client:
                    logger.error(f"Cd2助手连接失败，请检查配置：{_cd2_name}")
                    continue
                _client = Client(str(cd2_conf).split("#")[1], str(cd2_conf).split("#")[2],
                                 str(cd2_conf).split("#")[3])
                if not _client:
                    logger.error("Cd2助手连接失败，请检查配置")
                    continue
            ...
            # 立即运行一次
            if self._onlyonce:
                logger.info(f"Cd2助手定时任务，立即运行一次")
    ...
    def __send_notify(self, msg):
        ...
        self.post_message(title="Cd2助手通知",
                          mtype=mtype,
                          text=msg)
