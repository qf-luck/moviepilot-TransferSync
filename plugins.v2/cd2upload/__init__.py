...
class UploadQueue:
    ...
    def retry_task(self, task: UploadTask, max_attempts: int = 5, base_delay: int = 2, max_delay: int = 300,
...
class UploadStatistics:
    ...
    def record_upload_result(self, file_path: str, success: bool, duration: float = 0, file_size: int = 0,
                             error_type: str = None):
        """记录上传结果"""
        today = datetime.now().strftime('%Y-%m-%d')
        hour = datetime.now().strftime('%Y-%m-%d %H:00')
        ...
        with self.lock:
            ...
            if success:
                ...
                if duration > 0:
                    ...
                    self.performance_stats['avg_upload_time'] = (current_avg * (
                                total_success - 1) + duration) / total_success
            else:
    ...
    def get_performance_summary(self) -> Dict:
        ...
        with self.lock:
            ...
            return {
                ...
                'success_rate': round(total_success / (total_success + total_failed) * 100, 2) if (
                                                                                                              total_success + total_failed) > 0 else 0,
    ...
    def _check_cd2_clients_health(self) -> Dict:
        ...
        try:
            ...
            return {"status": "healthy",
                    "details": {"healthy_clients": healthy_clients, "total_clients": total_clients}}
...
class APIHandler:
    ...
    def _handle_health(self, method: str, params: Dict, headers: Dict) -> Dict:
        ...
        health_status = self.plugin._health_checker.check_health() if self.plugin._health_checker else {
            "status": "disabled"}
...
class Cd2Upload(_PluginBase):
    ...
    def _execute_task_with_lock(self, task_id: str, media_info: MediaInfo = None, meta: MetaBase = None,
        ...
        try:
            ...
            # 检查配额
            if self._quota_manager and not self._quota_manager.check_quota("upload_requests",
                                                                           len(waiting_process_list)):
    ...
    def _process_upload_directly(self, waiting_process_list: List[str], media_info: MediaInfo = None,
                                 meta: MetaBase = None, start_time: float = None):
        """直接处理上传（传统方式）"""
        ...
        logger.info(
            f"上传任务完成 - 成功: {upload_stats['success']}, 失败: {upload_stats['failed']}, 用时: {upload_stats['duration']}秒")
    ...
    def _handle_failed_upload(self, task: UploadTask):
        ...
        if task.retry_count < max_attempts:
            ...
            logger.info(
                f"任务重新加入队列进行重试: {task.file_path} (重试次数: {task.retry_count}, 下次重试: {retry_time})")
        else:
    ...
    def get_audit_trail(self, hours: int = 24) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "log_files": {
                    "business": str(
                        self._enterprise_logger.log_dir / f"{self._enterprise_logger.plugin_name}_business.log"),
                    "audit": str(self._enterprise_logger.log_dir / f"{self._enterprise_logger.plugin_name}_audit.log"),
                    "performance": str(
                        self._enterprise_logger.log_dir / f"{self._enterprise_logger.plugin_name}_performance.log"),
    ...
    def _send_notification(self, title: str, text: str = None, image: str = None):
        ...
        try:
            mtype = NotificationType.__getitem__(
                self._notification_type) if self._notification_type else NotificationType.Plugin
